...
const TaskList = (props) => {
  ...
  const discardHandler = item => {
    ...
    setTaskList(prevState => prevState.filter(task => task.name !== item.taskName))
  }
  ...
  // taskListFromStorage = taskListFromStorage.filter((task) => task.status === "todo")

  // console.log(taskListFromStorage)

  // setTaskList(taskListFromStorage)
